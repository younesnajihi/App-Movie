        ...
        <div class="container mx-auto px-4 flex flex-col md:flex-row items-center justify-between px-4 py-6">
            <ul class="flex flex-col md:flex-row items-center">
                ...
                <li class="md:ml-16 mt-3 md:mt-0">
                    <Link :href="route('movies')"  class="hover:text-gray-300">Movies</Link>
                </li>
                
                <li class="md:ml-6 mt-3 md:mt-0">
                    <Link :href="route('actors')" class="hover:text-gray-300">Actors</Link>
                </li>
                 <li class="md:ml-6 mt-3 md:mt-0">
                    <Link :href="route('tvs')" class="hover:text-gray-300">TV Shows</Link>
                </li>
            </ul>
            <div class="flex flex-col md:flex-row items-center">
               <div class="relative" v-if='isOpen' >
                   <input class="bg-gray-800 rounded-full w-64 px-4 py-1 text-sm" v-model.trim="search" @keyup="getMovie" placeholder="Search" type="text">
                    <div class="absolute bg-gray-800 rounded w-64 mt-4 " v-if='isOpen'>
                         <ul>
                            
                             <div v-for='movie in this.movies.results' class="overflow-auto">
                             <li class="border-b border-gray-700  " >
                                 <Link :href="route('show.movie',movie.id)" class="block hover:bg-gray-700 px-3 py-3">{{movie['title']}} </Link>
                                 </li>
                                 </div>
...
export default {
    ...
    methods :{
         getMovie() {
            const moviess =   fetch("https://api.themoviedb.org/3/search/movie?api_key=712e2d9abb302fc20c26079c28737fda&query="+this.search);
                    moviess.then(response =>{return response.json()})
                    .then((m) => { console.log(this.movies = m) });
